for pointing out defects and suggesting enhancements, if you have a question about one of the tools please refer to StackOverflow instead.

## Tech Stack

Here's a curated list of packages that you should have knowledge of, before starting your awesome project. However, the best way to have a complete list of dependencies is to see [package.json](https://gitlab.emdeon.net/react-ui/app-boilerplate/blob/develop/README.md).
...
- [...`redux-saga` or `reselect`](https://gitlab.emdeon.net/react-ui/app-boilerplate/blob/develop/docs/js/remove.md).
- ...[offline-first, add to homescreen, performant web font loading and image optimisation](https://gitlab.emdeon.net/react-ui/app-boilerplate/blob/develop/docs/general/remove.md)
- [`sanitize.css`](https://gitlab.emdeon.net/react-ui/app-boilerplate/blob/develop/docs/css/remove.md)
...
Like any other webpage your app starts with the [`app/index.html`](https://gitlab.emdeon.net/react-ui/app-boilerplate/blob/develop/app/index.html) file. React will render your application into the `div#app` .
...
Webpack requires an entry point to your application. Think of it as a door to your source code. In this boilerplate [`app/app.js`](https://gitlab.emdeon.net/react-ui/app-boilerplate/blob/develop/app/app.js) is that entry point. Webpack will access the entire app from this file, transpile the application into ES5 and create small chunks of transpiled code. Only the required chunks will be loaded in the browser so that you don't have to worry about the size of your application.
...
- A Router is set up, with all of your routes. See [`routes.js`](https://gitlab.emdeon.net/react-ui/app-boilerplate/blob/develop/app/routes.js)
...
- `ReactDOM.render()` not only renders the [root react component](https://gitlab.emdeon.net/react-ui/app-boilerplate/blob/develop/app/views/AppWrapper/components/AppWrapper.jsx) called `<App />`, of your application, but it renders it with `<Provider />`, `<LanguageProvider />` and `<Router />`.
...
`<Router />` sets up your routes. Check out [`routes.js`](https://gitlab.emdeon.net/react-ui/app-boilerplate/blob/develop/app/routes.js) to see how route `path`s are mapped with application containers.
...
The Redux `store` is the heart of your application. Check out [`createStore.js`](https://gitlab.emdeon.net/react-ui/app-boilerplate/blob/develop/app/store/createStore.js) to see how we have configured the store.
...
Run `npm start` to launch the application. If you start browsing at [https://localhost:3000](https://localhost:3000), by default you will be navigated to the home page. Here, notice that route is `"/"`, so the [`<HomePage />`](https://gitlab.emdeon.net/react-ui/app-boilerplate/blob/develop/app/containers/HomePage/index.js) container will be mounted. It is responsible for rendering a form with a textbox and a list of repositories.
...
Check out [`HomePage/sagas.js`](https://gitlab.emdeon.net/react-ui/app-boilerplate/blob/develop/app/containers/HomePage/sagas.js). It can be confusing for untrained eyes. The API of `redux-saga` is self-descriptive once you've seen it, so let's go over what happens in there:
...
- If the API successfully returns some data, a `reposLoaded()` action will be dispatched which carries the data. When redux store receives this action, [a reducer](https://gitlab.emdeon.net/react-ui/app-boilerplate/blob/develop/app/containers/App/reducer.js) will set incoming data in the new state tree.
